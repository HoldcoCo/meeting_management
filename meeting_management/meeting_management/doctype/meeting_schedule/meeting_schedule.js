...
cur_frm.set_query("contact", function () {
  return {
    ...
    filters: {
      link_doctype: cur_frm.doc.party_type,
...
frappe.ui.form.on("Meeting Schedule", {
  refresh: function (frm) {
    frm.add_custom_button(__("Create Meeting"), function () {
      frappe.model.open_mapped_doc({
        method:
          "meeting_management.meeting_management.doctype.meeting_schedule.meeting_schedule.make_meeting",
        frm: frm,
      });
    });
  },
    ...
    if (frm.doc.scheduled_from && !frm.doc.scheduled_to) {
      ...
      frm.set_value(
        ...
        frappe.datetime.get_datetime_as_string(
          frappe.datetime
            .str_to_obj(frm.doc.scheduled_from)
            .setHours(
              frappe.datetime.str_to_obj(frm.doc.scheduled_from).getHours() + 1
            )
        )
      );
    }
  ...
  contact_person_name: function (frm) {
    if (frm.doc.contact || frm.doc.contact_person_name) {
      let datetime = frappe.datetime
        .global_date_format(frm.doc.scheduled_from)
        .split(", ");
      let date = datetime[0] || "";
      let time = datetime[1] + "." || "";
      ...
      let message =
        "<p>Dear " +
        frm.doc.contact_person_name +
        ",</p><p>Greeting from " +
        frm.doc.company +
        "</p><p> This is regarding the telephonic conversation that we had" +
        ". The" +
        is_online +
        ...
        " at " +
        time +
        "</p><p>In case of any changes in your schedule, we request you to inform us timely.</p><p></p>Thanks & Regards,<p>" +
        sign +
        "</p>";
