...
var calculate_total_expense = function (frm) {
  var total_expense =
    flt(frm.doc.local_travel_expense) +
    flt(frm.doc.train_tickets) +
    flt(frm.doc.flight_ticket) +
    flt(frm.doc.food_expense) +
    flt(frm.doc.lodging_cost);
...
frappe.ui.form.on("Meeting", {
  send_mail: function (frm) {
    if (frm.is_dirty()) {
      ...
      frappe.call({
        method:
          "meeting_management.meeting_management.doctype.meeting.meeting.send_mail",
        args: {
          self: frm.doc,
        },
      });
  ...
  refresh: function (frm) {
    ...
    frm.fields_dict.meeting_party_representative.grid.get_field(
      "contact"
  ...
  on_submit: async function (frm) {
    for (const actionable of frm.doc.actionables) {
      // get user email from
      const allocatedTo = await frappe.db.get_doc(
        "Employee",
        actionable.responsible
      );
      await frappe.db
        .insert({
          doctype: "ToDo",
          ...
          status: "Open",
          priority: "Medium",
          date: actionable.expected_completion_date,
          description: actionable.actionable,
          allocated_to: allocatedTo.user_id,
    ...
    if (frm.doc.meeting_from && !frm.doc.meeting_to) {
      ...
      frm.set_value(
        ...
        frappe.datetime.get_datetime_as_string(
          frappe.datetime
            .str_to_obj(frm.doc.meeting_from)
            .setHours(
              frappe.datetime.str_to_obj(frm.doc.meeting_from).getHours() + 1
            )
        )
      );
    }
  ...
  party: function (frm) {
    ...
    frappe.call({
      method:
        "meeting_management.meeting_management.doctype.meeting_schedule.meeting_schedule.get_party_details",
      args: {
        party: frm.doc.party,
  ...
  total_kms: function (frm) {
    if (
      frappe.meta.get_docfield("Meeting", "total_kms") &&
      frappe.meta.get_docfield("Meeting", "rate_per_km")
    ) {
      frm.set_value(
        "local_travel_expense",
        flt(frm.doc.total_kms) * flt(frm.doc.rate_per_km)
      );
    }
  },
